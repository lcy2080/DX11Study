...
bool TextureShaderClass::InitializeShader(ID3D11Device *device, HWND hwnd, WCHAR *vsFilename, WCHAR *psFilename)
{
    ...
    result = D3DCompileFromFile(vsFilename, NULL, NULL, "TextureVertexShader", "vs_5_0", flags, 0, &vertexShaderBuffer, &errorMessage);
    if(FAILED(result))
    {
        //if the shader failed to compile it shoul dhave writen something to the error message.
        if (errorMessage)
        {
    ...
    result = D3DCompileFromFile(psFilename, NULL, NULL, "TexturePixelShader", "ps_5_0", flags, 0, &pixelShaderBuffer, &errorMessage);
    if (FAILED(result))
    {
    ...
    //Cretae The texture sampler state.
